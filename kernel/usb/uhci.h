...
public:
    explicit uhci_transfer(uhci &uhciRef, std::size_t size);
...
private:
    std::shared_ptr<usb_transfer> CreateTransferWithLock(bool commitTransaction, std::shared_ptr<usb_buffer> buffer, uint32_t size, usb_transfer_direction direction, bool bufferRounding, int8_t dataToggle, std::function<void (uhci_transfer &transfer)> &applyFunc);
    std::shared_ptr<usb_transfer> CreateTransferWithoutLock(bool commitTransaction, std::shared_ptr<usb_buffer> buffer, uint32_t size, usb_transfer_direction direction, bool bufferRounding, int8_t dataToggle, std::function<void (uhci_transfer &transfer)> &applyFunc);
public:
    ...
    std::shared_ptr<usb_transfer> CreateTransfer(bool commitTransaction, void *data, uint32_t size, usb_transfer_direction direction, std::function<void ()> doneCall, bool bufferRounding = false, uint16_t delayInterrupt = TRANSFER_NO_INTERRUPT, int8_t dataToggle = 0) override;
