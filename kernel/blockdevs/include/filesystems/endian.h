...
#ifndef FSBITS_ENDIAN_H
#define FSBITS_ENDIAN_H
...
template <typename T> constexpr T reverse_byte_order_per_word(T input) {
    if constexpr(sizeof(T) <= 1) {
        return input;
    } else {
        T output{0};
        T fac{1};
        for (int i = 0; i < (sizeof(T) / 2); i++) {
            output += ((T) reverse_byte_order<uint16_t>((uint16_t) ((input / fac) % 0x10000))) * fac;
            fac *= 0x10000;
        }
        return output;
    }
...
static_assert(reverse_byte_order_per_word('A') == 'A');
static_assert(reverse_byte_order_per_word<uint16_t>(0x1234) == 0x3412);
static_assert(reverse_byte_order_per_word<uint32_t>('ABCD') == 'BADC');
static_assert(reverse_byte_order_per_word<uint64_t>(0x1234567887654321) == 0x3412785665872143);
...
template <typename T> class little_endian {
private:
    T value;
public:
    constexpr little_endian() noexcept : value(0) {
    }
    constexpr little_endian(T value) noexcept : value() {
        assign(value);
    }
    constexpr T assign(const T &value) noexcept {
        if constexpr(sizeof(T) == 1 || my_platform_is_little_endian()) {
            this->value = value;
        } else if constexpr(my_platform_is_middle_endian()) {
            this->value = reverse_byte_order_per_word<T>(value);
        } else {
            this->value = reverse_byte_order<T>(value);
        }
        return value;
    ...
    constexpr T unpack() const noexcept {
        if constexpr(sizeof(T) == 1 || my_platform_is_little_endian()) {
            ...
            return reverse_byte_order_per_word<T>(value);
        } else {
            return reverse_byte_order<T>(value);
        }
    }
...
static_assert(little_endian<uint8_t>(0x41).Raw() == 'A');
static_assert(little_endian<uint16_t>(0x4142) == 0x4142);
static_assert(!my_platform_is_little_endian() || little_endian<uint16_t>(0x4142).Raw() == 0x4142);
static_assert(!my_platform_is_little_endian() || little_endian<uint32_t>(0x41424344).Raw() == 0x41424344);

#endif //FSBITS_ENDIAN_H
